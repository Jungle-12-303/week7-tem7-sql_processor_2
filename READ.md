...
### 고민 1. 왜 `id`만 인덱스로 처리했나

모든 컬럼에 인덱스를 붙이면 더 빨라 보일 수 있습니다. 하지만 MVP에서는 오히려 핵심이 흐려집니다.

#### 현재 답

가장 설명 가치가 큰 지점을 하나 고정했습니다.

- `id`: PK이므로 인덱스에 가장 자연스럽다
- `name`, `age`: 비교용 선형 탐색 대상으로 둔다
...
### 고민 4. 왜 범용 SQL 파서를 만들지 않았나

처음부터 범용 파서를 만들면 토크나이저, AST, 문법 규칙, 예외 처리까지 급격히 복잡해집니다.

#### 현재 답

이번 구현은 고정 패턴 SQL만 지원합니다.

- `INSERT INTO users VALUES (...)`
- `SELECT * FROM users ...`

즉 파서의 범용성보다, **SQL 처리 흐름과 인덱스 동작 이해**를 우선했습니다.

### 고민 5. 다음 단계로는 어디까지 확장할 수 있나

현재는 메모리 기반 고정 스키마입니다. 하지만 구조를 보면 자연스럽게 다음 단계가 보입니다.

#### 현재 답

확장 방향은 아래처럼 생각할 수 있습니다.

1. 토크나이저/AST 기반 SQL 파서
2. 고정 스키마가 아닌 테이블 메타데이터 분리
3. 메모리 저장에서 디스크 저장으로 확장
4. `name`, `age` 인덱스 추가
5. `DELETE`, `UPDATE`, `CREATE TABLE` 지원

즉 현재 구조는 작은 MVP이지만, **DB 엔진으로 확장되는 최소 뼈대**를 보여줍니다.
